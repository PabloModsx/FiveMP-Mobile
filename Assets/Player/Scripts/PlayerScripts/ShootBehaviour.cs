...
public class ShootBehaviour : GenericBehaviour
{
	...
	private bool isShotBtnPressed = false;
	...
	public void HandleStartFiring(bool z)
	{
		isShotBtnPressed = z;
        if (z && !isShooting && activeWeapon > 0 && burstShotCount == 0)
        {
            isShooting = true;
            ShootWeapon(activeWeapon);
        }
        else if (isShooting && !z)
        {
            isShooting = false;
        }
    }
			...
			if (shotDecay <= 0.4f)
			{
				...
				if (activeWeapon > 0)
				{
					...
					if (shotDecay <= (0.4f - 2 * Time.deltaTime))
					{
						...
						if (weapons[activeWeapon].mode == InteractiveWeapon.WeaponMode.AUTO && isShotBtnPressed)
						{
